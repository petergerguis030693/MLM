	...
	var dlabChartlist = function () {
		...
		var chartBar = function () {
			var options = {
			  ...
			  xaxis: {
				position: 'bottom',
				categories: [],  // Dynamische Kategorien (Labels)
				labels: {
				  style: {
					colors: '#787878',
					fontSize: '13px',
			  ...
			  tooltip: {
				y: {
				  formatter: function(val) {
					return "€ " + val.toFixed(2); // Keine zusätzlichen "000,00" mehr
				  }
				}
			  }
			};
			...
			if (jQuery("#chartBar").length > 0) {
			  ...
			  // Funktion zum Aktualisieren des Diagramms anhand des Zeitrahmens
				...
				fetch('/api/chart-data?timeframe=' + timeframe)
				  ...
				  .then(data => {
					chartBar1.updateOptions({
					  ...
					  series: [
						{ name: 'Erfolgreiche Zahlungen', data: data.successfulPayments },
						{ name: 'Fehlgeschlagene Zahlungen', data: data.failedPayments },
						{ name: 'Abgeschlossene Bestellungen', data: data.completedOrders },
						{ name: 'Offene Bestellungen', data: data.pendingOrders }
					  ]
					});
				  })
				  .catch(error => {
			  ...
			  // Standardmäßig monatliche Daten laden
			  updateChart('monthly');
		  
			  // Event-Listener für die Tabs
			  jQuery('.card-tabs .nav-link').on('click', function (e) {
				e.preventDefault();  // Standardverhalten unterbinden
				var timeframe = jQuery(this).attr("href").replace("#", "");
				// Falls "today" (für "Heute") verwendet wird, mappe es auf "daily"
				if (timeframe === "today") {
				  timeframe = "daily";
				}
				updateChart(timeframe);
				// Setze die aktive Klasse auf den aktuellen Tab
				jQuery('.card-tabs .nav-link').removeClass('active');
